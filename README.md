# BARIBA SIM — Expert 3v3

逆轉バリバリバース／バリバコイン Expert 規則 3v3 的非官方瀏覽器模擬器。
...
1. 玩家 1、玩家 2 各選三枚硬幣；每副 deck 的總 OX 不可超過 15,000。
2. 選擇猜拳勝者，再由勝者選 ORDER / XTREME。
3. 執行コイントス決定先攻。
4. 輪到你時，可選擇任何一枚仍在場內的己方硬幣，向後拖曳並放開；射擊方向與拖曳方向相反。
...
目前共 63 個 Node 測試，涵蓋資料、Expert deck OX、3v3 開局與勝負、OX 規則、メタバリビィ攻防、物理碰撞、リバースポット、翻面與 match state machine。
